...
public class EditState : ObjectState<WorldTileMap>
{
  ...
  private void HandleMouseMotion(InputEvent @event)
  {
    ...
    objectState.DrawRectangle(WorldTileMap.PREVIEW_LAYER, true); // erase the preview
    objectState.DrawRectangle(WorldTileMap.PREVIEW_LAYER);
  }
  ...
  private void HandleLeftClickInput()
  {
    if (Input.IsActionJustPressed("left_click"))
    {
      objectState.initialTilePosition = objectState.MouseToMap();
    }

    if (Input.IsActionJustReleased("left_click"))
    {
      if (objectState.initialTilePosition == objectState.MouseToMap())
      {
        PlaceOrEraseTile();
      }
  ...
  private void HandleRightClickInput()
  {
    if (Input.IsActionJustPressed("right_click"))
    {
      ...
      {
        // cancel the preview
        objectState.DrawRectangle(WorldTileMap.PREVIEW_LAYER, true);
        objectState.initialTilePosition = null;
        objectState.previousPreviewTiles.Clear();
      }
      else
  ...
  private void PlaceOrEraseTile()
  {
    ...
    {
      objectState.EraseCell(objectState.EditModeLayer, objectState.initialTilePosition.Value);
    }
    else
    {
      objectState.SetCell(objectState.EditModeLayer, objectState.initialTilePosition.Value, objectState.SourceId, (Vector2I)objectState.AtlasIndex);
    }
  }
